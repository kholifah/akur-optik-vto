...
export default function VTOCanvas({ imageSrc, alt, frameWidthMm, modelUrl, modelOffset }: Props) {
    ...
    async function setupFaceMesh(selectedDeviceId?: string | null) {
      ...
      try {
        ...
        if (name === 'NotFoundError' || name === 'OverconstrainedError') {
          ...
          // refresh device list
          try { const list = await navigator.mediaDevices.enumerateDevices(); setDevices(list.filter(d => d.kind === 'videoinput')) } catch { /* ignore */ }
  ...
  }, [modelUrl, modelOffset, deviceId, frameWidthMm])
